...
import { getStandableFloorState, isHazardTile, getBlockingWallHeight } from "../../world";
...
export function validateMovement(
  ...
  const rad = 10;
  const obstacleH = getBlockingWallHeight(newPos.x, newPos.y, rad);
  if (obstacleH > currentPos.z + 8.5) {
      console.log(
        `[Vanguard-2026] NoClip detectado para ${username} em obstaculo alto`
      );
      return { valid: false };
  }
